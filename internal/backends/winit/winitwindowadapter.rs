    ...
    for WinitWindowAdapter<Renderer>
{
    ...
    fn apply_geometry_constraint(
        ...
        self.with_window_handle(&mut |winit_window| {
            ...
            let into_size2 = |s: LogicalSize| -> winit::dpi::Size {
                let l = winit::dpi::LogicalSize::new(s.width, s.height);
                if let Some(f) = scale_factor_override {
                    l.to_physical::<f32>(f).into()
                } else {
                    l.into()
                }
            };
            ...
            let winit_min_inner = min_size.map(into_size2);
            winit_window.set_min_inner_size(winit_min_inner);
            let winit_max_inner = max_size.map(into_size2);
